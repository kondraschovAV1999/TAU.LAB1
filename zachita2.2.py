...
class Creature:  # Создаем исходный класс
    ...
    def do_action(self, unit_list):  # метод класса, отвечающий за дейсвтия персонажа
        enemy = []
        ...
        if not enemy:
            ...
            self.attack(unit_list, target=enemy[0])

    def attack(self, unit_list, target):  # метод для атаки персонажем
        damage = roll_dice(self.power)
        print(f'{self.get_name()} deal {damage} damage to {target.get_name()}')
        target.get_damage(damage, unit_list)
    ...
    def heal(self, unit_list):
        print(self.get_name(), 'лечит всех союзников')
        for unit in unit_list:
            if unit.fraction == self.fraction and not unit.is_defeat:
                unit.current_HP += self.power
                if unit.current_HP > unit.maxHP:
                    unit.current_HP = unit.maxHP
...
class Dwarf(Creature):
    ...
    def attack(self, unit_list, target):  # Переопределяем родительский метод
        damage = self.damage
        print(f'{self.get_name()} deal {damage} damage to {target.get_name()}')
        target.get_damage(damage, unit_list)
...
goblinList = [Goblin('Klo-Klo', 'evil'), Goblin('Klak-Klak', 'evil'), Goblin('Bax-Bax', 'evil'),
              Goblin('Klic', 'evil'), Goblin('Klo', 'evil'), Goblin('Brok', 'evil'), Goblin('Ktock', 'evil'),
              Goblin('Rat', 'evil'), Goblin('Krock', 'evil')]
humanList = [Human('Boris', 'Hero'), Human('Vitya', 'Hero'), Human('Petya', 'Hero')]
