...
class Creature:  # Создаем исходный класс
    ...
    def attack(self, target):  # метод для атаки персонажем
        damage = roll_dice(self.power)
        print(f'{self.get_name()} deal {damage} damage to {target.get_name()}')
        target.get_damage(damage)

    def get_damage(self, count):  # метод отвечающий за получение урона, и смерть персонажа
...
class Dwarf(Creature):
    ...
    def attack(self, target):  # Переопределяем родительский метод
        damage = self.damage
        print(f'{self.get_name()} deal {damage} damage to {target.get_name()}')
        target.get_damage(damage)
